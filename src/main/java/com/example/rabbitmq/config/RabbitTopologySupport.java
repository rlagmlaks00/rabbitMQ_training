...
final class RabbitTopologySupport {

  private RabbitTopologySupport() {
  }

  // 실패 시 공용 DLX(order.dlx)로 데드레터링되는 quorum queue
  static Queue deadLetteredQueue(String name) {
  ...
  static Queue plainQueue(String name) {
  ...
  static Binding binding(String exchange, String queue, String routingKey) {
